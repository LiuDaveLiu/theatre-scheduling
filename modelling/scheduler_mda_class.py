...
class TheatreScheduler:
    def __init__(self, case_file_path, session_file_path, case_no):
        """
...
#         model.CASES = pe.Set(initialize=self.df_cases_g["CaseID"].tolist())
        ...
        model.CASE_NO = pe.Set(self.case_no)
        ...
        # Cases can be assigned to a maximum of one session
        def session_assignment_1(model, case, session):
            return sum([model.SESSION_ASSIGNED[(case, session)] for case, session in self.case_no]) == 0
        model.SESSION_ASSIGNMENT_1 = pe.Constraint(model.TASKS, rule=session_assignment_1)
